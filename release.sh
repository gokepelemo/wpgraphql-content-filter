...
create_package() {
    ...
    # Ensure we have production dependencies only
    if command -v composer > /dev/null && [ -f "composer.json" ]; then
        print_status "Installing production dependencies for self-contained package..."
        composer install --no-dev --no-interaction --optimize-autoloader
    ...
    if [ -d "$package_dir/tests" ] || [ -f "$package_dir/composer.json" ]; then
    ...
    if [ -d "$package_dir/vendor" ]; then
        print_success "Production dependencies included in package"
        # Check for our required libraries
        if [ -d "$package_dir/vendor/league/html-to-markdown" ] && [ -d "$package_dir/vendor/ezyang/htmlpurifier" ]; then
            print_success "Required runtime libraries found: league/html-to-markdown, ezyang/htmlpurifier"
        else
            print_warning "Some required runtime libraries may be missing"
        fi
    else
        print_warning "No vendor directory found - plugin may not work without manual dependency installation"
    fi
    ...
    print_status "Package contains production dependencies and is ready for deployment"
